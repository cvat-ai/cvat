...
2. Install necessary packages with a package manager and cvat-cli.
For example YOLO needs libxcb1, libgl1, libglib2.0-0
3. If you need any Python dependencies, you may put them to requirements.txt
4. You should have `func.py` file that acts as adapter between your agent and CVAT.
You can refer to existing adapters for YOLO, SAM2 or transformers for inspiration. (`/ai-models/detector/yolo/func.py`, `/ai-models/tracker/sam2/func.py`, `/ai-models/transformers/func.py`)
...
Please refer to shell scripts [`entrypoint.sh`](./entrypoint.sh), [`function_registration.sh`](./function_registration.sh) and [`function_deregistration.sh`](./function_deregistration.sh) for
examples of how to use cvat-cli to implement these capabilities.

### General recommendations
- It is recommended to run your container with a non-root user.
