...
// How close the region's leading edge gets to the viewport edge before the
// viewport starts panning to follow it. 0 = pan only after the region
// physically crosses the viewport edge. Small positive values give the user a
// bit of anticipation before the scroll kicks in.
...
export function attachRegionAutoScroll(
    ...
    // Pan the viewport so the moving region stays visible. The viewport
    // follows the region — scroll speed equals the region's movement speed,
    // which equals the user's mouse speed. No independent rAF loop, no
    // edge-zone speed ramp; one single source of velocity.
    const followRegion = (
        region: Region,
        sc: HTMLElement,
        direction: number,
        ...
        sc.scrollLeft = Math.max(0, Math.min(maxScroll, sc.scrollLeft + scrollDelta));
    };
    ...
    const onPointerMoveCapture = (e: PointerEvent): void => {
        ...
        // Plugin's bubble-phase listener would also fire with deltaX=0 (clientX
        // is frozen by pointer lock), but stopping it removes the redundant
        // emit('update') round-trip.
    ...
    const requestLock = (): void => {
        ...
        try {
            ...
            /* ignore — fall back to no-lock mode */
        }
    };
