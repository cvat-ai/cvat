...
function makeMessage(label: Label, labelType: State['labelType'], pointsCount: number): JSX.Element {
    const labelTypeTranslations: Partial<Record<LabelType, string>> = {
        [LabelType.RECTANGLE]: '矩形',
        [LabelType.POLYGON]: '多边形',
        [LabelType.POLYLINE]: '折线',
        [LabelType.POINTS]: '点',
        [LabelType.CUBOID]: '立方体',
        [LabelType.ELLIPSE]: '椭圆',
        [LabelType.MASK]: '掩码',
        [LabelType.SKELETON]: '骨架',
    };

    const readableLabelType = labelTypeTranslations[labelType as LabelType] ?? String(labelType);
    let readableShape = '';
    if (labelType === LabelType.POINTS) {
        ...
        readableShape = `一个 ${readableLabelType}`;
    }
...
const componentShortcuts = {
    ...
    DELETE_OBJECT_SINGLE_SHAPE: {
        name: '删除对象',
        description: '删除活动对象。使用 Shift 强制删除锁定的对象',
                        ...
                        <Row justify='start' className='cvat-single-shape-annotation-sidebar-finish-frame-wrapper'>
                            <Col>
                                {typeof state.nextFrame === 'number' ? (
                                    <Button size='large' onClick={() => finishOnThisFrame(false)}>跳过</Button>
                                ) : (
