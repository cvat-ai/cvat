            ...
            return (
                <>
                    <Form.Item
                        label='访问密钥 ID'
                        name='key'
                        rules={[{ required: true, message: '请输入您的访问密钥 ID' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
                            maxLength={128}
                    ...
                    <Form.Item
                        label='秘密访问密钥'
                        name='secret_key'
                        rules={[{ required: true, message: '请输入您的秘密访问密钥' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
                            maxLength={64}
            ...
            return (
                <>
                    <Form.Item
                        label='账户名称'
                        name='account_name'
                        rules={[{ required: true, message: '请输入您的账户名称' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
                            minLength={3}
                    ...
                    <Form.Item
                        label='SAS 令牌'
                        name='SAS_token'
                        rules={[{ required: true, message: '请输入您的 SAS 令牌' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
        ...
        if (providerType === ProviderType.AZURE_CONTAINER && credentialsType === CredentialsType.CONNECTION_STRING) {
            return (
                <Form.Item
                    ...
                    rules={[{ required: true, message: '请输入您的连接字符串' }]}
                    {...internalCommonProps}
                >
                    <Input.Password
                        maxLength={1024}
            ...
            return (
                <Form.Item
                    ...
                    label={(
                        <CVATTooltip
                            title={(
                                '你可以上传密钥文件；如果留空，将使用环境变量 GOOGLE_APPLICATION_CREDENTIALS。'
                            )}
                        >
                            密钥文件
                            <Button
        ...
        return (
            <>
                ...
                <Form.Item
                    ...
                    rules={[{ required: true, message: '请指定凭证类型' }]}
                    {...internalCommonProps}
                >
                ...
                <Form.Item
                    label='端点 URL'
                    help='使用兼容 S3 的云存储 API 时，您可以为存储指定一个端点'
                    name='endpoint_url'
                    {...internalCommonProps}
                >
                    <Input />
