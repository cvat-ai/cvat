...
export default function LabelSelector(props: Props): JSX.Element {
    ...
    // Use the v5 `options` prop instead of <Select.Option> children. With
    // `virtual={false}` + JSX option children, antd's render path stalls
    // the popover entry animation in `ant-zoom-big-appear-prepare`
    // (verified via bisect — see audio popover tests). The options-array
    // path uses a different internal renderer that doesn't trigger the
    // hang.
