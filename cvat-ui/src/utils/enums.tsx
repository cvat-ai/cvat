// Copyright (C) 2021-2022 Intel Corporation
//
// SPDX-License-Identifier: MIT

export enum ProviderType {
    AWS_S3_BUCKET = 'AWS_S3_BUCKET',
    AZURE_CONTAINER = 'AZURE_CONTAINER',
    GOOGLE_CLOUD_STORAGE = 'GOOGLE_CLOUD_STORAGE',
}

export enum CredentialsType {
    KEY_SECRET_KEY_PAIR = 'KEY_SECRET_KEY_PAIR',
    ACCOUNT_NAME_TOKEN_PAIR = 'ACCOUNT_NAME_TOKEN_PAIR',
    ANONYMOUS_ACCESS = 'ANONYMOUS_ACCESS',
    CONNECTION_STRING = 'CONNECTION_STRING',
    KEY_FILE_PATH = 'KEY_FILE_PATH',
}

export enum StorageStatuses {
    AVAILABLE = 'AVAILABLE',
    FORBIDDEN = 'FORBIDDEN',
    NOT_FOUND = 'NOT_FOUND',
}

// if this enum is changed, kindly update the conflictDetector and it's helpers as well
export enum ShortcutScope {
    GLOBAL = 'GLOBAL',
    ANNOTATION_PAGE = 'ANNOTATION_PAGE',
    CONTROLS_SIDE_BAR = 'CONTROLS_SIDE_BAR',
    OBJECTS_SIDE_BAR = 'OBJECTS_SIDE_BAR',

    STANDARD_WORKSPACE = 'STANDARD_WORKSPACE',
    STANDARD_WORKSPACE_CONTROLS = 'STANDARD_WORKSPACE_CONTROLS',

    STANDARD_3D_WORKSPACE = 'STANDARD_3D_WORKSPACE',
    STANDARD_3D_WORKSPACE_CONTROLS = 'STANDARD_3D_WORKSPACE_CONTROLS',

    ATTRIBUTE_ANNOTATION_WORKSPACE = 'ATTRIBUTE_ANNOTATION_WORKSPACE',
    ATTRIBUTE_ANNOTATION_WORKSPACE_CONTROLS = 'ATTRIBUTE_ANNOTATION_WORKSPACE_CONTROLS',

    REVIEW_WORKSPACE = 'REVIEW_WORKSPACE',
    REVIEW_WORKSPACE_CONTROLS = 'REVIEW_WORKSPACE_CONTROLS',

    SINGLE_SHAPE_WORKSPACE = 'SINGLE_SHAPE_WORKSPACE',
    SINGLE_SHAPE_WORKSPACE_CONTROLS = 'SINGLE_SHAPE_WORKSPACE_CONTROLS',

    TAG_ANNOTATION_WORKSPACE = 'TAG_ANNOTATION_WORKSPACE',
    TAG_ANNOTATION_WORKSPACE_CONTROLS = 'TAG_ANNOTATION_WORKSPACE_CONTROLS',

    LABELS_EDITOR = 'LABELS_EDITOR',
}
