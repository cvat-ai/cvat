...
function pushAudioUndo(
    ...
    diffs: AudioRegionDiff[],
    activeRegionIdAfter: string | null,
): { undo: AudioHistoryEntry[]; redo: AudioHistoryEntry[] } {
    if (diffs.length === 0 && state.audioPlayer.activeRegionId === activeRegionIdAfter) {
        return state.audioHistory;
    }
    const entry: AudioHistoryEntry = {
        actionName,
        diffs,
        activeRegionIdBefore: state.audioPlayer.activeRegionId,
        activeRegionIdAfter,
    };
    const undo = [...state.audioHistory.undo, entry].slice(-MAX_AUDIO_HISTORY);
    return { undo, redo: [] };
}
...
export default (state = defaultState, action: AnyAction): AnnotationState => {
    switch (action.type) {
        ...
        case AnnotationActionTypes.SET_AUDIO_REGIONS: {
            const diffs = diffRegions(state.audioPlayer.regions, action.payload.regions);
            return {
                ...state,
                audioHistory: pushAudioUndo(
                    state,
                    describeDiffs(diffs),
                    diffs,
                    state.audioPlayer.activeRegionId,
                ),
                audioPlayer: {
                    ...state.audioPlayer,
        ...
        case AnnotationActionTypes.UPDATE_AUDIO_REGION_ATTRIBUTE: {
            ...
            const before = state.audioPlayer.regions.find((r) => r.id === regionId);
            ...
            return {
                ...state,
                audioHistory: pushAudioUndo(state, 'Changed attribute', diffs, state.audioPlayer.activeRegionId),
                audioPlayer: {
                    ...state.audioPlayer,
                    regions: state.audioPlayer.regions.map((r) => (r.id === regionId ? after : r)),
                    hasUnsavedChanges: true,
                },
            };
        }
        ...
        case AnnotationActionTypes.TOGGLE_AUDIO_REGION_LOCK: {
            ...
            return {
                ...state,
                audioHistory: pushAudioUndo(state, 'Toggled lock', diffs, state.audioPlayer.activeRegionId),
                audioPlayer: {
                    ...state.audioPlayer,
                    regions: state.audioPlayer.regions.map((r) => (r.id === regionId ? after : r)),
                    hasUnsavedChanges: true,
        ...
        case AnnotationActionTypes.TOGGLE_AUDIO_REGION_HIDDEN: {
            ...
            return {
                ...state,
                audioHistory: pushAudioUndo(state, 'Toggled visibility', diffs, state.audioPlayer.activeRegionId),
                audioPlayer: {
                    ...state.audioPlayer,
                    regions: state.audioPlayer.regions.map((r) => (r.id === regionId ? after : r)),
                    hasUnsavedChanges: true,
        ...
        case AnnotationActionTypes.SAVE_AUDIO_ANNOTATIONS_SUCCESS: {
            return {
                ...
                audioPlayer: {
                    ...state.audioPlayer,
                    hasUnsavedChanges: false,
                    version: action.payload.version ?? state.audioPlayer.version,
                },
            };
        }
        ...
        case AnnotationActionTypes.AUDIO_UNDO: {
            const undoStack = [...state.audioHistory.undo];
            ...
            return {
                ...
                audioHistory: {
                    undo: undoStack,
                    redo: [...state.audioHistory.redo, entry].slice(-MAX_AUDIO_HISTORY),
                },
                audioPlayer: {
                    ...state.audioPlayer,
                    regions: applyReverseDiffs(state.audioPlayer.regions, entry.diffs),
        ...
        case AnnotationActionTypes.AUDIO_REDO: {
            const redoStack = [...state.audioHistory.redo];
            ...
            return {
                ...state,
                audioHistory: {
                    undo: [...state.audioHistory.undo, entry].slice(-MAX_AUDIO_HISTORY),
                    redo: redoStack,
                },
                audioPlayer: {
                    ...state.audioPlayer,
                    regions: applyForwardDiffs(state.audioPlayer.regions, entry.diffs),
