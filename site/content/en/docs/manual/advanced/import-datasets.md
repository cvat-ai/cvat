...
1. Choose the format for exporting the dataset. Exporting and importing is available in:
   - Standard CVAT formats:
     - [CVAT for video]({{< relref "/docs/manual/advanced/xml_format#interpolation" >}})
       choose if the task is created in interpolation mode.
     - [CVAT for images]({{< relref "/docs/manual/advanced/xml_format#annotation" >}})
       choose if a task is created in annotation mode.

       <br>
   - And also in formats from the [list of annotation formats supported by CVAT]({{< relref "/docs/manual/advanced/formats" >}}).
...
1. You can choose a storage for dataset export by selecting a target storage `Local` or `Cloud storage`.
   ...
   Learn more about [attach cloud storage]({{< relref "/docs/manual/basics/attach-cloud-storage" >}}).
