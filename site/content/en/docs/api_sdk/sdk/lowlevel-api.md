...
and provides access to different APIs.
...
Each operation provides metainfo in the `<operation>_endpoint` member. It also allows
to call the method with better control on invocation details.

For a typical server entity like `Task`, `Project`, `Job` etc. the `*Api` classes provide methods
that reflect Create-Read-Update-Delete (CRUD) operations:
...
endpoints called `list_...`. When there are lots of data, the responses can be paginated to
...
for this, so the solution is to write a loop over to collect and join data from multiple requests.
