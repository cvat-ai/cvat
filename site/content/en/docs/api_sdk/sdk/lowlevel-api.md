...
- `_request_timeout` (`None | int | float | Tuple[int | float, int | float]`) -
  Allows to control timeout. If one number provided, it will be total request timeout. It can also
...
- `_content_type` (`None | str`) - Allows to specify the `Content-Type` header value
  ...
  Read more about file uploads [here](#sending-data). Default `None`, which is to try to detect
  automatically.
