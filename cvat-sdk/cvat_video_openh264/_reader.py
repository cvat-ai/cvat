...
"""Proof-of-concept reader for the MP4/H.264 chunks generated by CVAT."""
...
class UnsupportedVideoChunkError(ValueError):
    """Raised when a chunk is outside the narrow CVAT MP4/H.264 PoC contract."""
...
def _parse_avcc(data: bytes) -> _AvcConfiguration:
    ...
    if profile != 66 or not profile_compatibility & 0x40:
        raise UnsupportedVideoChunkError(
            "The PoC supports only constrained-baseline H.264 video chunks"
        )
...
def _validate_composition_offsets(file: BinaryIO, stbl: _Box) -> None:
    ...
    if any(offset for _, offset in struct.iter_unpack(f">I{offset_format}", table)):
        raise UnsupportedVideoChunkError(
            "The PoC does not support H.264 chunks with display-order reordering"
        )
...
def iter_frames(
    ...
    *,
    ...
    This PoC deliberately has no codec downloader or general-purpose media fallback.
    """
