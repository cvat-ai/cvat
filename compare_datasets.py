...
class CvatExtractor(Extractor):
    ...
    def _parse_meta(context):
        ...
        for ev, el in context:
            if ev == 'start':
                ...
                elif accepted('label', 'parent', next_state='label_parent'): pass
        ...
        for label, params in labels.items():
            ...
            if parent and parent not in labels:
                raise ValueError(f"Label's '{label}' parent label '{parent}' is not found")
    ...
    def _parse_shape_ann(cls, ann, categories, *, parent_label: str = None):
        ...
        label = ann.get('label')
        label_id = categories[AnnotationType.label].find(label, parent=parent_label or '')[0]
        if label_id is None:
            raise ValueError(
        ...
        if ann_type == 'polyline':
            ...
            for element in ann.get('elements', []):
                elements.append(cls._parse_shape_ann(
...
def OKS(a, b, sigma=None, bbox=None, scale=None, visibility=None):
    ...
    return np.sum(visibility * np.exp(-(dists**2) / (2 * scale * (2 * sigma) ** 2))) / np.sum(visibility)
...
class _DistanceComparator(dm.ops.DistanceComparator):
    def __init__(self, categories: dm.CategoriesInfo, **kwargs):
    ...
    def _get_skeleton_info(self, skeleton_label_id: int):
        ...
        if skeleton_info is None:
            ...
            skeleton_info = sorted(
                idx for idx, label in enumerate(label_cat)
            ...
            for skeleton in source:
                ...
                merged_points = dm.Points(
                    points=list(itertools.chain.from_iterable(
                        p.points if p else [0, 0]
                        for p in skeleton_points
                    )),
                    visibility=list(itertools.chain.from_iterable(
...
class _Comparator:
    def __init__(self, categories: dm.CategoriesInfo):
        ...
        self._annotation_comparator = _DistanceComparator(categories)
...
class DatasetComparator:
    ...
    def _generate_frame_annotation_conflicts(
        self, frame_id, frame_results, this_item: dm.DatasetItem, gt_item: dm.DatasetItem
        ...
        for shape_type in [
            ...
            dm.AnnotationType.points, dm.AnnotationType.polygon,
            dm.AnnotationType.polyline, dm.AnnotationType.skeleton,
        ]:
        ...
        for gt_ann, this_ann in matches:
            ...
            attr_matches, attr_mismatches, attr_gt_extra, attr_this_extra = \
            ...
            compared_attributes_count += \
                len(attr_matches) + len(attr_mismatches) + len(attr_gt_extra) + len(attr_this_extra)
            valid_annotations_count += 0 == (
        ...
        total_annotations_count = len(this_item.annotations) + len(gt_item.annotations)
        ...
        self._per_frame_results.setdefault(frame_id, {}).update({
            ...
            'attribute_accuracy': valid_attributes_count / compared_attributes_count,
            'annotation_accuracy': valid_annotations_count / compared_annotations_count,
            'overall_accuracy': \
                (valid_attributes_count + valid_annotations_count) / \
                ((compared_annotations_count + compared_attributes_count) or 1),
            'error_count': len(conflicts), # assuming no more than 1 error per annotation
    ...
    def prepare_report(self) -> AnnotationConflictsReport:
        ...
        for item in self._this_dataset:
            this_annotations_count += len(item.annotations)

            for ann in item.annotations:
                this_attributes_count += \
                    len(set(ann.attributes).difference(self._comparator.ignored_attrs))

        this_properties_count = this_annotations_count + this_attributes_count
        ...
        mean_annotation_accuracy = valid_attributes_count / (compared_attributes_count or 1)
        mean_attribute_accuracy = valid_annotations_count / (compared_annotations_count or 1)

        estimated_dataset_error_count = int((1 - mean_accuracy) * this_properties_count)
        estimated_dataset_invalid_annotations_count = \
            int((1 - mean_annotation_accuracy) * this_annotations_count)
        estimated_dataset_invalid_attributes_count = \
            int((1 - mean_attribute_accuracy) * this_attributes_count)
        ...
        return AnnotationConflictsReport(
            ...
            this_total_annotations_count=this_properties_count,
...
def main(args=None):
    ...
    class JsonConverter(json.JSONEncoder):
        def default(self, o: Any):
            if isinstance(o, (AnnotationConflict ,AnnotationConflictsReport, AnnotationId)):
                return o.to_dict()
            else:
