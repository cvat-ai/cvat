...
class TaskObjects(IDerivedMetric):
    ...
    def calculate(self):
        ...
        for job_report in self._primary_statistics:
            ...
            for c_entry, u_entry, d_entry in zip(dataseries["created"], dataseries["updated"], dataseries["deleted"]):
                entry = combined_statistics.setdefault(parser.parse(c_entry["datetime"]).date(), {
