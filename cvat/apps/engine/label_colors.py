...
DEFAULT_COLORMAP_PATH = osp.join(osp.dirname(__file__), "predefined_label_colors.txt")


def _get_color_from_index(index):
...
def _parse_default_colors(file_path=None):
...
def _generate_color(color, used_colors):
...
def get_label_color(label_name, label_colors):
    ...
    if color is None:
        ...
        color = _get_color_from_index(name_hash)

    if color in label_colors:
        color = _generate_color(color, used_colors)
