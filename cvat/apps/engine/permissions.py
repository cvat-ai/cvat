...
class DownloadExportedExtension:
    ...
    @staticmethod
    def extend_params_with_rq_job_details(
    ...
    def extend_resource_with_rq_job_details(self, data: dict[str, Any]) -> None:
        data["rq_job"] = {"owner": {"id": self.rq_job_id.user_id if self.rq_job_id else None}}
...
class ServerPermission(OpenPolicyAgentPermission):
    ...
    @classmethod
    def create(
        cls, request: ExtendedRequest, view: ViewSet, obj: None, iam_context: dict[str, Any]
...
class UserPermission(OpenPolicyAgentPermission):
    ...
    @classmethod
    def create(
        cls, request: ExtendedRequest, view: ViewSet, obj: User | None, iam_context: dict[str, Any]
    ):
        permissions = []
        for scope in cls.get_scopes(request, view, obj):
            self = cls.create_base_perm(request, view, scope, iam_context, obj)
...
class CloudStoragePermission(OpenPolicyAgentPermission):
    ...
    def create_scope_view(
        cls, iam_context: dict[str, Any], storage_id: int, request: ExtendedRequest | None = None
    ):
        try:
    ...
    def get_resource(self):
        ...
        if self.scope.startswith("create"):
            ...
            data = {
                ...
                "organization": (
                    {"id": self.obj.organization_id} if self.obj.organization_id else None
                ),
            }
...
class ProjectPermission(
    ...
    def create(
        ...
        for field_source, field in [
            ...
            if cloud_storage_id := _get_key(field_source, field_path):
                permissions.append(
                    CloudStoragePermission.create_scope_view(
    ...
    def get_scope_specific_params(
        ...
        *,
        ...
        if (
            scope
            in (cls.Scopes.EXPORT_ANNOTATIONS, cls.Scopes.EXPORT_BACKUP, cls.Scopes.EXPORT_DATASET)
            and obj
        ):
...
class TaskPermission(
    ...
    @classmethod
    def create(
        cls, request: ExtendedRequest, view: ViewSet, obj: Task | None, iam_context: dict[str, Any]
    ) -> list[OpenPolicyAgentPermission]:
        permissions = []
    ...
    @classmethod
    def create_scope_view(
        cls, request: ExtendedRequest, task: int | Task, iam_context: dict[str, Any] | None = None
    ):
                ...
                "project": (
                    {
                        ...
                        "organization": (
                            {"id": project.organization_id} if project.organization_id else None
                        ),
                    }
...
class JobPermission(OpenPolicyAgentPermission, DownloadExportedExtension):
    ...
    @classmethod
    def create(
        cls, request: ExtendedRequest, view: ViewSet, obj: Job | None, iam_context: dict[str, Any]
    ) -> list[OpenPolicyAgentPermission]:
        permissions = []
    ...
    @classmethod
    def create_scope_view(
        cls, request: ExtendedRequest, job: int | Job, iam_context: dict[str, Any] | None = None
    ):
    ...
    def get_resource(self):
        ...
        if self.obj:
            ...
            data = {
                "organization": {"id": organization_id},
                "task": {"owner": {"id": task.owner_id}, "assignee": {"id": task.assignee_id}},
                "project": (
                    {
...
class CommentPermission(OpenPolicyAgentPermission):
    ...
    def create(
        ...
        for scope in cls.get_scopes(request, view, obj):
            self = cls.create_base_perm(
                request, view, scope, iam_context, obj, issue_id=request.data.get("issue")
            )
            permissions.append(self)
    ...
    def get_resource(self):
        ...
        if self.obj:
            ...
            data.update({"id": self.obj.id, "owner": {"id": getattr(self.obj.owner, "id", None)}})
...
class IssuePermission(OpenPolicyAgentPermission):
    ...
    @classmethod
    def create(
        cls, request: ExtendedRequest, view: ViewSet, obj: Issue | None, iam_context: dict[str, Any]
    ) -> list[OpenPolicyAgentPermission]:
        permissions = []
        assignee_id = request.data.get("assignee")
        for scope in cls.get_scopes(request, view, obj):
...
class LabelPermission(OpenPolicyAgentPermission):
    ...
    @classmethod
    def create(
        cls, request: ExtendedRequest, view: ViewSet, obj: Label | None, iam_context: dict[str, Any]
    ) -> list[OpenPolicyAgentPermission]:
        Scopes = cls.Scopes
        ...
        for scope in cls.get_scopes(request, view, obj):
            if scope in [Scopes.DELETE, Scopes.UPDATE, Scopes.VIEW]:
                ...
                permissions.append(
                    JobPermission.create_base_perm(
                        request,
                        view,
                        JobPermission.Scopes.VIEW,
                        iam_context,
                        obj=obj,
                    )
                )
    ...
    def create(
        ...
        for scope in cls.get_scopes(request, view, obj):
            if scope == Scopes.VIEW and isinstance(obj, AnnotationGuide):
                permissions.append(
                    AnnotationGuidePermission.create_base_perm(
                ...
                try:
                    ...
                    permissions.append(
                        AnnotationGuidePermission.create_base_perm(
                            request,
                            view,
                            AnnotationGuidePermission.Scopes.UPDATE,
                            iam_context,
                            obj=obj,
                        )
                    )
