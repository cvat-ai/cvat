...
_OLD_WRAPPER_FUNC_PATH = "cvat.apps.engine.utils.import_resource_with_clean_up_after"


def _migrate_job(job: Job, logger: Logger) -> None:
    if job.func_name != _OLD_WRAPPER_FUNC_PATH:
        return

    real_func, *rest_args = job.args
    new_func_name = f"{real_func.__module__}.{real_func.__qualname__}"

    logger.info(
        "Rewriting job %s: %s -> %s",
        job.id,
        _OLD_WRAPPER_FUNC_PATH,
        new_func_name,
    )
    job.func_name = new_func_name
    job.args = tuple(rest_args)
    job.save()
