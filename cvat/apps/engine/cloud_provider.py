...
class BackblazeB2(AWS_S3):
    ...
    def __init__(
        ...
        *,
        ...
        if not endpoint_url:
            raise Exception(
                "Backblaze B2 requires an endpoint_url (e.g., https://s3.us-east-005.backblazeb2.com)"
            )
    ...
    def create(self):
        ...
        try:
            ...
            response = self._bucket.create(ACL="private", ObjectLockEnabledForBucket=False)
            slogger.glob.info(f"Backblaze B2 bucket '{self.name}' has been created")
    ...
    def upload_file(self, file_path: str, key: str | None = None, /):
        ...
        try:
            ...
            self._log_event(
                "file_upload_error",
                {"bucket": self.name, "key": key or os.path.basename(file_path), "error": str(ex)},
            )
            raise
    ...
    def upload_fileobj(self, file_obj: BinaryIO, key: str, /):
        ...
        try:
            ...
            self._log_event("fileobj_upload", {"bucket": self.name, "key": key})
        except Exception as ex:
            self._log_event(
                "fileobj_upload_error", {"bucket": self.name, "key": key, "error": str(ex)}
            )
            raise
    ...
    def download_file(self, key: str, path: str, /):
        ...
        try:
            ...
            self._log_event("file_download", {"bucket": self.name, "key": key, "path": path})
        except Exception as ex:
            self._log_event(
                "file_download_error", {"bucket": self.name, "key": key, "error": str(ex)}
            )
            raise
    ...
    def delete_file(self, file_name: str, /):
        ...
        try:
            ...
            self._log_event("file_delete", {"bucket": self.name, "key": file_name})
        except Exception as ex:
            self._log_event(
                "file_delete_error", {"bucket": self.name, "key": file_name, "error": str(ex)}
            )
            raise
