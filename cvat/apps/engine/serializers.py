...
class AnnotationSerializer(serializers.Serializer):
    ...
    def _validate_id_for_create(self, value):
        if value is not None:
            raise serializers.ValidationError("must be omitted")
        return value

    def _validate_id_for_update(self, value):
        if value is None:
    ...
    def validate_id(self):
        ...
        if action := self.context.get("annotation_action"):
            if action == PatchAction.CREATE:
                return self._validate_id_for_create
            elif action == PatchAction.UPDATE or action == PatchAction.DELETE:
                return self._validate_id_for_update
            else:
