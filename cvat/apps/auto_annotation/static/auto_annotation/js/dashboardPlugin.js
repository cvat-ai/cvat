...
class AutoAnnotationModelManagerView {
    constructor() {
                ...
                <div class="regular" id="${window.cvatUI.autoAnnotation.uploadContentId}">
                    ...
                    <table style="border-spacing: 0 1em;">
                        ...
                        <tr>
                            ...
                            <td>
                                <input id="${window.cvatUI.autoAnnotation.uploadLocalSourceId}" type="radio" name="modelSourceType" value="local">
                                <label for="${window.cvatUI.autoAnnotation.uploadLocalSourceId}" class="regular h3"> Local </label>
                                <br>
                                <input id="${window.cvatUI.autoAnnotation.uploadShareSourceId}" type="radio" name="modelSourceType" value="shared" checked>
                                <label for="${window.cvatUI.autoAnnotation.uploadShareSourceId}" class="regular h3"> Share </label>
                            </td>
                        </tr>
                        <tr>
                            <td> <label class="regular h3">Model Type: </label> </td>
                            <td>
                                <input id="${window.cvatUI.autoAnnotation.uploadModelTypeOpenVinoId}" type="radio" name="modelType" value="openvino">
                                <label for="${window.cvatUI.autoAnnotation.uploadModelTypeOpenVinoId}" class="regular h3"> OpenVINO </label>
                                <br>
                                <input id="${window.cvatUI.autoAnnotation.uploadModelTypeTFAnnotationId}" type="radio" name="modelType" value="tfannotation" checked>
                                <label for="${window.cvatUI.autoAnnotation.uploadModelTypeTFAnnotationId}" class="regular h3"> TF Annotation </label>
                            </td>
                        </tr>
        ...
        this.openVinoSource = this.el.find(`#${window.cvatUI.autoAnnotation.uploadModelTypeOpenVinoId}`);
        this.tfAnnotationSource = this.el.find(`#${window.cvatUI.autoAnnotation.uploadModelTypeTFAnnotationId}`);
        ...
        this.modelType = this.openVinoSource.prop('checked') ? 'openvino' : 'tfannotation';
        this.files = [];
        ...
        function validateFiles(isUpdate, files, source, modelType) {
            //openvino by default
            let extensions = ['xml', 'bin', 'py', 'json'];
            if(modelType === 'tfannotation') {
                extensions = ['csv','pb'];
            }
        ...
        this.openVinoSource.on('click', () => {
            if (this.modelType !== 'openvino') {
                this.modelType = 'openvino';
            }
        });

        this.tfAnnotationSource.on('click', () => {
            if (this.modelType !== 'tfannotation') {
                this.modelType = 'tfannotation';
            }
        });
        ...
        this.submitButton.on('click', () => {
            try {
                ...
                let validatedFiles = {};
                try {
                    validatedFiles = validateFiles(this.id !== null, this.files, this.source, this.modelType);
                } catch (err) {
                    this.uploadMessage.css('color', 'red');
                ...
                modelData.append('model_type',this.modelType);
                modelData.append('shared', this.globallyBox.prop('checked'));

                ['xml', 'bin', 'json', 'py','csv','pb'].filter(e => e in validatedFiles).forEach((ext) => {
    ...
    reset() {
        ...
        this.localSource.prop('checked', false);
        this.tfAnnotationSource.prop('checked', true);
...
class AutoAnnotationModelRunnerView {
    ...
    reset(data, initButton) {
        ...
        window.cvatUI.autoAnnotation.data.openvinoModels.forEach((model) => {
...
window.cvatUI.autoAnnotation = {
    ...
    uploadModelTypeOpenVinoId: 'annotatorManagerUploadModelTypeOpenVino',
    uploadModelTypeTFAnnotationId: 'annotatorManagerUploadModelTypeTFAnnotation',
    ...
    uploadModelTypeOpenVinoSelectorId: 'annotatorManagerUploadModelTypeOpenVinoSelector',
    uploadModelTypeTFAnnotationSelectorId: 'annotatorManagerUploadModelTypeTFAnnotationSelector',
