...
MIGRATION_BLUEPRINT_MODULE_PATH = f"cvat.apps.engine.redis_migrations.001_cleanup_scheduled_jobs"

MIGRATION_FILENAME_FORMAT = '{:03}_{}.py'
...
class TestRedisMigrations(TestCase):
    ...
    def generate_migration_file(app_path: PosixPath, migration_name: str, number: int = 0) -> PosixPath:
        ...
        filename = MIGRATION_FILENAME_FORMAT.format(number, migration_name)
    ...
    def _init_test_migration(
        self, app_name: str, migration_name: str, runner: Callable[[MigrationType], None]
    ):

        def path_to_module(path: PosixPath) -> str:
            return str(path).removesuffix(".py").replace("/", ".")

        def app_module_to_label(appModule: str):
            return appModule.rpartition(".")[2]

        # def init_migration_mock(mock: Mock, app_path: PosixPath, migration_name: str):
        #     app_module_path = path_to_module(app_path)
        #     app_label = app_module_to_label(app_module_path)
        #     self.test_migration = mock
        #     # self.test_migration.__class__ = BaseMigration
        #     self.test_migration.app_label = app_label
        #     self.test_migration.name = migration_name
        #     self.test_migration.run = runner
        ...
        migration_file_path = TestRedisMigrations.generate_migration_file(app_path, migration_name)
        mock_migration_module_path = path_to_module(migration_file_path)
        # obj_path = f"{mock_migration_module_path}.{MIGRATION_CLASS_NAME}"

        # print(migration_file_path)
        ...
        print("INIT")
        # init_migration_mock(mock, app_path, migration_name)


    @classmethod
    def setUpClass(cls):
        cls.Blueprint = get_class_from_module(MIGRATION_BLUEPRINT_MODULE_PATH, MIGRATION_CLASS_NAME)
        # cls.redis = fakeredis.FakeRedis()
