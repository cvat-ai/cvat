...
def create_thread(tid, labels_mapping, user, model_path, num_c):
	try:
...
def create(request, tid, mid):
	...
	try:
		...
		# # COCO Labels,  kept here as it helps with debugging
		# auto_segmentation_labels = { "BG": 0,
		...
		queue.enqueue_call(func=create_thread,
			args=(tid, labels_mapping, request.user, seg_model_file_path, num_c),
