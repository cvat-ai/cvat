...
def create_arg_parser() -> ArgumentParser:
    ...
    parser.add_argument('-g', '--gen-dir', default=None,
        help="The directory with test generators (default: the default location)")
    parser.add_argument('-o', '--output-dir', default='.', type=osp.abspath,
...
def call_generator(module_path: str, gen_params: Namespace):
    subprocess.check_call(
        ['python3', module_path, gen_params.config_dir],
        cwd=gen_params.output_dir
    )
...
def main(args: Optional[Sequence[str]] = None) -> int:
    ...
    with ThreadPoolExecutor() as pool:
        pool.map(partial(call_generator, gen_params=args),
