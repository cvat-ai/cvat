...
OWNERSHIPS = ['project:owner', 'project:assignee', 'task:owner', 'task:assignee',
    'job:assignee', 'issue:owner', 'issue:assignee', 'owner', 'none']
    ...
    with open(f'{name}_test.gen.rego', 'wt') as f:
        ...
        for scope, context, ownership, privilege, membership, same_org, has_proj in product(
            ...
            for resource in RESOURCES(scope):
                if not is_valid(scope, context, ownership, privilege, membership, resource, same_org, has_proj):
                    continue

                data = get_data(scope, context, ownership, privilege, membership, resource, same_org)
                test_name = get_name(scope, context, ownership, privilege, membership, resource, same_org, has_proj)
