...
class ProjectAnnotation:
    ...
    def add_task(self, task_fields: dict, files: dict, project_data: ProjectData):
        ...
        db_task = TaskWriteSerializer.create(
            None, {**task_fields, "data_id": db_data.id, "project_id": self.db_project.id}
        )
    ...
    def export(
        ...
        **options,
        ...
        project_data = ProjectData(
            annotation_irs=self.annotation_irs, db_project=self.db_project, host=host
        )
        ...
        with TmpDirManager.get_tmp_directory() as temp_dir:
            try:
                importer(
                    ...
                    temp_dir,
                    project_data,
                    load_data_callback=self.load_dataset_data,
                    **options,
                )
