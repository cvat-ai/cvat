...
class InstanceLabelData:
    ...
    def __init__(self, instance: Task | Project) -> None:
        instance = (
            instance.project
            if isinstance(instance, Task) and instance.project_id is not None
            else instance
        )
        ...
        self._label_mapping: dict[int, Label] = {
            db_label.id: db_label for db_label in sorted(db_labels, key=lambda v: v.pk)
        }

        self._attribute_mapping = {
            db_label.id: {"mutable": {}, "immutable": {}, "spec": {}} for db_label in db_labels
        }

        for db_label in db_labels:
            for db_attribute in db_label.attributespec_set.all():
                if db_attribute.mutable:
                    self._attribute_mapping[db_label.id]["mutable"][
                        db_attribute.id
                    ] = db_attribute.name
                else:
                    self._attribute_mapping[db_label.id]["immutable"][
                        db_attribute.id
                    ] = db_attribute.name
...
class CommonData(InstanceLabelData):
            ...
            if db_label.type == str(LabelType.SKELETON):
                ...
                for db_sublabel in list(db_label.sublabels.all()):
                    label["svg"] = label["svg"].replace(
                        f'data-label-id="{db_sublabel.id}"', f'data-label-name="{db_sublabel.name}"'
                    )
    ...
    def _export_shape(self, shape):
        return CommonData.Shape(id=shape["id"], label_id=shape["label_id"])
    ...
    def _export_track(self, track, idx):
        track["shapes"] = list(
            filter(lambda x: not self._is_frame_deleted(x["frame"]), track["shapes"])
        )
    ...
    def _export_label(label):
        return CommonData.Label(id=label.id, name=label.name, color=label.color, type=label.type)
    ...
    def group_by_frame(
        ...
        get_tags_for_frame = make_getter_by_frame_for_annotation_stream(
            sorted(
                (tag for tag in self._annotation_ir.tags if tag["frame"] in included_frames),
    ...
    def _import_tag(self, tag):
        ...
        _tag["attributes"] = [
            ...
            or (self.soft_attribute_import and attrib.name not in CVAT_INTERNAL_ATTRIBUTES)
        ]
        return _tag
    ...
    def _import_shape(self, shape, parent_label_id=None):
        ...
        _shape["elements"] = [
            self._import_shape(element, label_id) for element in _shape.get("elements", [])
        ]
    ...
    def _import_track(self, track, parent_label_id=None):
        ...
        _track["frame"] = self.rel_frame_id(min(int(shape.frame) for shape in _track["shapes"]))
        _track["label_id"] = label_id
        _track["attributes"] = []
        _track["shapes"] = [shape._asdict() for shape in _track["shapes"]]
        _track["elements"] = [
            self._import_track(element, label_id) for element in _track.get("elements", [])
        ]
        for shape in _track["shapes"]:
            ...
            shape["attributes"] = [
                self._import_attribute(label_id, attrib, mutable=True)
                for attrib in shape["attributes"]
    ...
    def match_frame(
        self, path: str, root_hint: str | None = None, *, path_has_ext: bool = True
    ) -> int | None:
        if path_has_ext:
    ...
    def _init_meta(self):
        ...
        self._meta = {
            JobData.META_FIELD: {
                ...
                "start_frame": str(
                    self._db_data.start_frame + db_segment.start_frame * self._frame_step
                ),
                "stop_frame": str(
                    self._db_data.start_frame + db_segment.stop_frame * self._frame_step
                ),
                ...
                "owner": (
                    {"username": self._db_task.owner.username, "email": self._db_task.owner.email}
        ...
        if self._label_mapping is not None:
            self._meta[JobData.META_FIELD]["labels"] = CommonData._convert_db_labels(
    ...
    def _init_frame_info(self):
        ...
        if self.db_instance.segment.type == SegmentType.SPECIFIC_FRAMES:
            ...
            self._excluded_frames.update(
                frame for frame in self.rel_range if self.abs_frame_id(frame) not in frame_set
            )
        ...
        if self._required_frames:
            ...
            self._required_frames = set(
                frame for frame in self._required_frames if frame in rel_range
            )
...
class TaskData(CommonData):
    ...
    def meta_for_task(db_task, host, label_mapping=None):
        ...
        meta = {
            ...
            "segments": [
                (
                    "segment",
                    {
                        ...
                        "start": str(db_segment.start_frame),
                        "stop": str(db_segment.stop_frame),
                        "url": "{}/api/jobs/{}".format(host, db_segment.job_set.first().id),
                    },
                )
            ...
            "owner": (
                {"username": db_task.owner.username, "email": db_task.owner.email}
                if db_task.owner
                else ""
            ),
            "assignee": (
                {"username": db_task.assignee.username, "email": db_task.assignee.email}
                if db_task.assignee
                else ""
            ),
        }
    ...
    def _init_frame_info(self):
        ...
        if self.db_data.validation_mode == models.ValidationMode.GT_POOL:
            ...
            for db_image in self._get_db_images():
                ...
                self._frame_mapping[self._get_filename(db_image.path)] = self.rel_frame_id(
                    db_image.frame
                )
...
class ProjectData(InstanceLabelData):
    ...
    def __init__(
        ...
        *,
        ...
        self._frame_info: dict[tuple[int, int], Literal["path", "width", "height", "subset"]] = (
            dict()
        )
    ...
    def abs_frame_id(self, task_id: int, relative_id: int) -> int:
        ...
        return (
            relative_id * task.data.get_frame_step()
    ...
    def rel_frame_id(self, task_id: int, absolute_id: int) -> int:
        task = self._db_tasks[task_id]
        d, m = divmod(absolute_id - task.data.start_frame, task.data.get_frame_step())
    ...
    def _init_tasks(self):
        ...
        self._frame_steps: dict[int, int] = {
            task.id: task.data.get_frame_step() for task in self._db_tasks.values()
        }
    ...
    def _init_frame_info(self):
        ...
        self._deleted_frames = {
            (task.id, frame): True
            for task in self._db_tasks.values()
            for frame in task.data.deleted_frames
        }
        ...
        for task in self._db_tasks.values():
            ...
            if hasattr(task.data, "video"):
                ...
                self._frame_info.update(
                    {
                        (task.id, self.rel_frame_id(task.id, db_image.frame)): {
                            ...
                            "path": (
                                ...
                                if not db_image.is_placeholder
                                else db_image.path
                            ),
                            "id": db_image.id,
                            "width": db_image.width,
    ...
    def _init_meta(self):
        self._meta = {
            ProjectData.META_FIELD: {
                ...
                "tasks": [
                    ("task", TaskData.meta_for_task(db_task, self._host))
    ...
    def _export_tracked_shape(self, shape: dict, task_id: int):
        return ProjectData.TrackedShape(
            ...
            attributes=self._export_attributes(shape["attributes"]),
            elements=[
                self._export_tracked_shape(element, task_id)
    ...
    def _export_labeled_shape(self, shape: dict, task_id: int):
        return ProjectData.LabeledShape(
            ...
            attributes=self._export_attributes(shape["attributes"]),
            elements=[
                self._export_labeled_shape(element, task_id)
                for element in shape.get("elements", [])
            ],
            task_id=task_id,
        )
    ...
    def _export_track(self, track: dict, task_id: int, task_size: int, idx: int):
        track["shapes"] = list(
            filter(lambda x: (task_id, x["frame"]) not in self._deleted_frames, track["shapes"])
        )
        tracked_shapes = TrackManager.get_interpolated_shapes(
        ...
        return ProjectData.Track(
            ...
            source=track["source"],
            shapes=[
                self._export_tracked_shape(shape, task_id)
                for shape in tracked_shapes
                if (task_id, shape["frame"]) not in self._deleted_frames
            ],
            task_id=task_id,
            elements=[
                self._export_track(element, task_id, task_size, i)
                for i, element in enumerate(track.get("elements", []))
            ],
        )
                ...
                def fill_annotations(frame: CommonData.Frame, original_frame: CommonData.Frame):
                    def fix_anno_frame(shape):
                        ...
                        if hasattr(shape, "elements"):
                            shape._replace(
                                elements=[fix_anno_frame(element) for element in shape.elements]
                            )
                        return shape

                    frame.labeled_shapes = [
                        fix_anno_frame(shape) for shape in original_frame.labeled_shapes
                    ]
    ...
    def init_task_data(self, task_id: int) -> TaskData:
        ...
        task_data = TaskData(
            ...
            host=self._host,
            create_callback=(
                self._task_annotations[task_id].create
                if self._task_annotations is not None
                else None
            ),
        )
    ...
    def split_dataset(self, dataset: dm.Dataset):
        for task_id in self._db_tasks.keys():
            if task_id not in self.new_tasks:
                continue
            task_data = self._task_data(task_id)
            subset_dataset: dm.Dataset = dataset.subsets()[
                task_data.db_instance.subset
            ].as_dataset()
...
class MediaProvider3D(MediaProvider):
    ...
    class ImageFromLazyChunk(datumaro.components.media.ImageFromBytes):
        def __init__(self, path: str, *args, data_getter: Callable[[], bytes], **kwargs):
            kwargs.setdefault("ext", osp.splitext(path)[1])
            super().__init__(data_getter, *args, **kwargs)
            self._path = path
...
class CVATDataExtractorMixin:
    def __init__(self, *, convert_annotations: Callable = None):
        ...
        for _, label in labels:
            ...
            if label["type"] == str(LabelType.SKELETON):
                ...
                for el in ET.fromstring("<root>" + label.get("svg", "") + "</root>"):
                    if el.tag == "line":
                        joints.append(
                            [int(el.attrib["data-node-from"]), int(el.attrib["data-node-to"])]
                        )
                    elif el.tag == "circle":
...
class CvatDataExtractor(dm.DatasetBase, CVATDataExtractorMixin):
    def __init__(
        ...
        **kwargs,
        ...
        self._user = (
            self._load_user_info(instance_meta) if dimension == DimensionType.DIM_3D else {}
        )
    ...
    def _process_one_frame_data(self, frame_data: CommonData.Frame) -> dm.DatasetItem:
        dm_media_args = {
            ...
            "ext": self._ext_per_task[frame_data.task_id]
            or frame_data.name.rsplit(osp.extsep, maxsplit=1)[1],
        }
    ...
    def _read_cvat_anno(self, cvat_frame_anno: CommonData.Frame, labels: list):
        ...
        def map_label(name, parent=""):
            return label_cat.find(name, parent)[0]

        label_attrs = {
            label.get("parent", "") + label["name"]: label["attributes"] for _, label in labels
        }

        return self.convert_annotations(
            cvat_frame_anno, label_attrs, map_label, self._format_type, self._dimension
        )
...
class CvatToDmAnnotationConverter:
    ...
    def _convert_shape(
        self, shape: CommonData.LabeledShape, *, index: int
        ...
        if shape.type == ShapeType.POINTS:
            anno = dm.Points(
                dm_points, label=dm_label, attributes=dm_attr, group=dm_group, z_order=shape.z_order
            )
            ...
            anno = MaskConverter.cvat_rle_to_dm_rle(
                SimpleNamespace(
                    **{
                        "points": shape.points,
                        "label": dm_label,
                ...
                self.cvat_frame_anno.height,
                self.cvat_frame_anno.width,
            )
            ...
            for element in shape.elements:
                element_attr = self._convert_attrs(shape.label + element.label, element.attributes)
            ...
            anno = dm.Skeleton(
                elements, label=dm_label, attributes=dm_attr, group=dm_group, z_order=shape.z_order
            )
        else:
    ...
    def _convert_shapes(self, shapes: list[CommonData.LabeledShape]) -> Iterable[dm.Annotation]:
        ...
        self.num_of_tracks = reduce(
            lambda a, x: a + (1 if getattr(x, "track_id", None) is not None else 0), shapes, 0
        )
...
def convert_cvat_anno_to_dm(
    cvat_frame_anno, label_attrs, map_label, format_name=None, dimension=DimensionType.DIM_2D
) -> list[dm.Annotation]:
...
def match_dm_item(
    item: dm.DatasetItem, instance_data: ProjectData | CommonData, root_hint: str | None = None
) -> int:
    ...
    if not frame_number in instance_data.frame_info:
        raise CvatImportError("Could not match item id: " "'%s' with any task frame" % item.id)
    return frame_number
...
def import_dm_annotations(dm_dataset: dm.Dataset, instance_data: ProjectData | CommonData):
    ...
    for item in dm_dataset:
        ...
        if (
            isinstance(instance_data.db_instance, Job)
        ...
        for ann in item.annotations:
            if ann.type in shapes:
                ...
                if group is None:
                    ...
                    group_size[ann.group] += 1
        group_map = {g: s for g, s in group_size.items() if 1 < s and group_map[g]}
        ...
        for idx, ann in enumerate(item.annotations):
            try:
                ...
                if ann.type in shapes:
                    ...
                    occluded = (
                        datumaro.util.cast(ann.attributes.pop("occluded", None), to_bool) is True
                    )
                    keyframe = (
                        datumaro.util.cast(ann.attributes.get("keyframe", None), to_bool) is True
                    )
                    outside = (
                        datumaro.util.cast(ann.attributes.pop("outside", None), to_bool) is True
                    )

                    track_id = ann.attributes.pop("track_id", None)
                    source = (
                        ann.attributes.pop("source").lower()
                        if ann.attributes.get("source", "").lower() in sources
                    ...
                    if (
                        track_id is None
                        or "keyframe" not in ann.attributes
                        or dm_dataset.format not in track_formats
                    ):
                        elements = []
                        if ann.type == dm.AnnotationType.skeleton:
                            for element in ann.elements:
                                ...
                                element_occluded = (
                                    element.visibility[0] == dm.Points.Visibility.hidden
                                )
                                element_outside = (
                                    element.visibility[0] == dm.Points.Visibility.absent
                                )
                                element_source = (
                                    element.attributes.pop("source").lower()
                                    if element.attributes.get("source", "").lower() in sources
                                    else SourceType.FILE
                                )
                                elements.append(
                        ...
                        instance_data.add_shape(
                            instance_data.LabeledShape(
                                ...
                                occluded=occluded,
                                z_order=(
                                    ann.z_order if ann.type != dm.AnnotationType.cuboid_3d else 0
                                ),
                                group=group_map.get(ann.group, 0),
                                source=source,
                                rotation=rotation,
                        ...
                        if ann.type == dm.AnnotationType.skeleton:
                            for element in ann.elements:
                                ...
                                if element.label not in tracks[track_id]["elements"]:
                                    tracks[track_id]["elements"][element.label] = (
                                        instance_data.Track(
                                            label=label_cat.items[element.label].name,
                                            group=0,
                                            source=source,
                                ...
                                tracks[track_id]["elements"][element.label].shapes.append(
                                    instance_data.TrackedShape(
                                        type=shapes[element.type],
                                        frame=frame_number,
                                        occluded=element_occluded,
                                        outside=element_outside,
    ...
    def _validate_track_shapes(shapes):
        ...
        def _close_last_interval(closing_shape):
            ...
            closing_shape = closing_shape._replace(
                outside=True, keyframe=True, frame=closing_shape.frame + instance_data.frame_step
            )
            keyframe_shapes.append(closing_shape)
        ...
        if (
            ...
            and (
                prev_shape.frame + instance_data.frame_step
                <= stop_frame
...
def import_labels_to_project(project_annotation: ProjectAnnotation, dataset: dm.Dataset) -> None:
    ...
    for label in dataset.categories()[dm.AnnotationType.label].items:
        db_label = Label(
            name=label.name, color=get_label_color(label.name, label_colors), type="any"
        )
...
def load_dataset_data(
        ...
        for label in dataset.categories()[dm.AnnotationType.label].items:
            if not project_annotation.db_project.label_set.filter(name=label.name).exists():
                if label.name == "background":
                    ...
                    continue
                raise CvatImportError(
                    f'Target project does not have label with name "{label.name}"'
                )
    for subset_id, subset in enumerate(dataset.subsets().values()):
        ...
        job_meta.progress = (subset_id + (job_meta.task_progress or 0.0)) / len(
            dataset.subsets().keys()
        )
        job_meta.save()
