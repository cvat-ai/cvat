        ...
        for shape in track["shapes"]:
            if prev_shape:
                ...
                if not prev_shape["outside"]:
                    shapes.extend(interpolator.interpolate_shape(prev_shape, shape))
...
class ShapeInterpolator:
    @classmethod
    def copy_shape(cls, source, frame, points=None, rotation=None):
    ...
    @classmethod
    def find_angle_diff(cls, right_angle, left_angle):
    ...
    @classmethod
    def simple_interpolation(cls, shape0, shape1):
        shapes = []
        ...
        for frame in range(shape0["frame"] + 1, shape1["frame"]):
            ...
            rotation = (shape0["rotation"] + cls.find_angle_diff(
                shape1["rotation"], shape0["rotation"],
            ) * offset + 360) % 360
            points = shape0["points"] + diff * offset

            shapes.append(cls.copy_shape(shape0, frame, points.tolist(), rotation))

        return shapes

    @classmethod
    def points_interpolation(cls, shape0, shape1):
        if len(shape0["points"]) == 2 and len(shape1["points"]) == 2:
            return cls.simple_interpolation(shape0, shape1)
        else:
            shapes = []
            for frame in range(shape0["frame"] + 1, shape1["frame"]):
                shapes.append(cls.copy_shape(shape0, frame))

        return shapes

    @classmethod
    def interpolate_position(cls, left_position, right_position, offset):
        def to_array(points):
            return np.asarray(
    ...
    @classmethod
    def polyshape_interpolation(cls, shape0, shape1):
        shapes = []
        ...
        for frame in range(shape0["frame"] + 1, shape1["frame"]):
            offset = (frame - shape0["frame"]) / distance
            points = cls.interpolate_position(shape0, shape1, offset)

            shapes.append(cls.copy_shape(shape0, frame, points))
    ...
    @classmethod
    def interpolate_shape(cls, shape0, shape1):
        ...
        if is_rectangle or is_cuboid or is_ellipse or is_skeleton:
            shapes = cls.simple_interpolation(shape0, shape1)
        elif is_points:
            shapes = cls.points_interpolation(shape0, shape1)
        elif is_polygon or is_polyline:
            shapes = cls.polyshape_interpolation(shape0, shape1)
        else:
            raise NotImplementedError()
