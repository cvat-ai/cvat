...
export default class AnnotationsSaver {
    ...
    constructor(version: number, collection, session: Task | Job) {
        ...
        this.initialObjects = { shapes: {}, tracks: {}, tags: {}, intervals: {} };
    ...
    async save(onUpdateArg?: (message: string) => void): Promise<void> {
        ...
        if (flush) {
            ...
            const findPair = (
                ...
                const labelID = objectToSave.label_id;
                const frameKey = 'frame' in objectToSave ? objectToSave.frame : ('start' in objectToSave ? objectToSave.start : undefined);

                // optimization to avoid stringifying each object in collection
                const potentialObjects = (collection as CollectionObject[]).filter(
                    (object) => {
                        const objFrame = 'frame' in object ? object.frame : ('start' in object ? object.start : undefined);
                        return objFrame === frameKey &&
                            object.label_id === labelID &&
                            !existingIDs.includes(object.id);
                    },
                );
