# Copyright (C) CVAT.ai Corporation
#
# SPDX-License-Identifier: MIT

import cv2
import numpy as np
import onnxruntime as ort
import yaml
from pathlib import Path


class ModelHandler:
    def __init__(self, labels):
        self.model = None
        self.labels = labels
        self.config = self.load_config()
        self.input_size = self.config.get('input_config', {}).get('imgsz', 256)
        self.conf_threshold = self.config.get('export_config', {}).get('conf_thres', 0.25)
        self.iou_threshold = self.config.get('export_config', {}).get('iou_thres', 0.45)
        self.load_network()

    def load_config(self):
        """Load model configuration from model_config.yaml file."""
        config_path = "model_config.yaml"
        try:
            with open(config_path, 'r') as f:
                config = yaml.safe_load(f)
            print(f"Loaded configuration from {config_path}")
            return config
        except FileNotFoundError:
            print(f"Warning: {config_path} not found. Using default values.")
            return {}
        except yaml.YAMLError as e:
            print(f"Warning: Error parsing {config_path}: {e}. Using default values.")
            return {}
        except Exception as e:
            print(f"Warning: Unexpected error loading {config_path}: {e}. Using default values.")
            return {}

    def load_network(self):
        """Load the ONNX model for inference."""
        model_name = self.config.get('model_info', {}).get('model_name', 'yolov8_sienz_oranges_color')
        model_path = f"{model_name}.onnx"

        try:
            # Set up execution providers (GPU first if available, then CPU)
            providers = ['CPUExecutionProvider']
            if ort.get_available_providers():
                available_providers = ort.get_available_providers()
                if 'CUDAExecutionProvider' in available_providers:
                    providers = ['CUDAExecutionProvider', 'CPUExecutionProvider']

            # Create ONNX runtime session
            so = ort.SessionOptions()
            so.log_severity_level = 3

            self.model = ort.InferenceSession(model_path, providers=providers, sess_options=so)
            self.input_details = [i.name for i in self.model.get_inputs()]
            self.output_details = [i.name for i in self.model.get_outputs()]

            # Verify we have 2 outputs (detection + masks)
            if len(self.output_details) != 2:
                raise ValueError(f"Expected 2 outputs (detection + masks), got {len(self.output_details)}")

            print(f"Loaded ONNX model: {model_path}")
            print(f"Execution provider: {self.model.get_providers()[0]}")
            print(f"Model outputs: {len(self.output_details)}")

        except Exception as e:
            raise Exception(f"Cannot load model {model_path}: {e}")

    def preprocess_image(self, image):
        """
        Preprocess image for ONNX inference.

        Args:
            image: PIL Image

        Returns:
            tuple: (preprocessed tensor, original shape)
        """
        # Convert PIL to numpy array
        image_np = np.array(image)
        original_shape = image_np.shape[:2]  # (height, width)

        # Resize to model input size
        resized_image = cv2.resize(image_np, (self.input_size, self.input_size))

        # Normalize to [0, 1]
        normalized_image = resized_image.astype(np.float32) / 255.0

        # Transpose to CHW format and add batch dimension
        input_tensor = np.transpose(normalized_image, (2, 0, 1))
        input_tensor = np.expand_dims(input_tensor, axis=0)

        return input_tensor, original_shape

    def apply_nms(self, boxes, scores, iou_threshold):
        """
        Apply Non-Maximum Suppression to filter overlapping detections.

        Args:
            boxes: Bounding boxes in format [x1, y1, x2, y2]
            scores: Confidence scores for each box
            iou_threshold: IoU threshold for NMS

        Returns:
            List of indices to keep after NMS
        """
        if len(boxes) == 0:
            return []

        # Convert to format expected by cv2.dnn.NMSBoxes
        boxes_xywh = []
        for box in boxes:
            x1, y1, x2, y2 = box
            w = x2 - x1
            h = y2 - y1
            boxes_xywh.append([x1, y1, w, h])

        # Apply NMS
        indices = cv2.dnn.NMSBoxes(
            boxes_xywh,
            scores.tolist(),
            self.conf_threshold,
            iou_threshold
        )

        # Handle both old and new OpenCV return formats
        if indices is not None:
            if isinstance(indices, np.ndarray) and len(indices) > 0:
                return indices.flatten().tolist()
            elif isinstance(indices, (list, tuple)) and len(indices) > 0:
                return list(indices) if isinstance(indices, tuple) else indices

        return []

    def scale_boxes(self, boxes, original_shape):
        """
        Scale boxes from model input size to original image size.

        Args:
            boxes: Boxes in model input coordinates
            original_shape: Original image shape (height, width)

        Returns:
            Scaled boxes
        """
        if len(boxes) == 0:
            return boxes

        orig_h, orig_w = original_shape
        scale_x = orig_w / self.input_size
        scale_y = orig_h / self.input_size

        scaled_boxes = boxes.copy()
        scaled_boxes[:, [0, 2]] *= scale_x  # x coordinates
        scaled_boxes[:, [1, 3]] *= scale_y  # y coordinates

        return scaled_boxes

    def generate_masks(self, mask_coeffs, proto_masks, boxes, original_shape):
        """
        Generate instance masks from mask coefficients and prototype masks.

        Args:
            mask_coeffs: Mask coefficients [N, 32]
            proto_masks: Prototype masks [32, 64, 64]
            boxes: Bounding boxes [N, 4] in original image coordinates
            original_shape: Original image shape (height, width)

        Returns:
            List of binary masks for each detection
        """
        if len(mask_coeffs) == 0:
            return []

        # Compute masks: mask = sigmoid(coeffs @ proto_masks)
        # mask_coeffs: [N, 32], proto_masks: [32, H, W] -> masks: [N, H, W]
        masks = np.matmul(mask_coeffs, proto_masks.reshape(32, -1))  # [N, H*W]
        masks = masks.reshape(-1, proto_masks.shape[1], proto_masks.shape[2])  # [N, H, W]

        # Apply sigmoid activation
        masks = 1 / (1 + np.exp(-masks))

        # Resize masks to original image size and crop to bounding boxes
        original_height, original_width = original_shape
        final_masks = []

        for i, (mask, box) in enumerate(zip(masks, boxes)):
            # Resize mask to original image size
            mask_resized = cv2.resize(mask, (original_width, original_height))

            # Convert box coordinates to integers
            x1, y1, x2, y2 = box.astype(int)

            # Ensure coordinates are within image bounds
            x1 = max(0, min(x1, original_width - 1))
            y1 = max(0, min(y1, original_height - 1))
            x2 = max(x1 + 1, min(x2, original_width))
            y2 = max(y1 + 1, min(y2, original_height))

            # Create final mask - zero outside bounding box
            final_mask = np.zeros((original_height, original_width), dtype=np.float32)
            final_mask[y1:y2, x1:x2] = mask_resized[y1:y2, x1:x2]

            # Apply threshold to create binary mask
            binary_mask = (final_mask > 0.5).astype(np.uint8)

            final_masks.append(binary_mask)

        return final_masks

    def mask_to_cvat_format(self, mask, box):
        """
        Convert binary mask to CVAT mask format.

        Args:
            mask: Binary mask (numpy array)
            box: Bounding box [x1, y1, x2, y2]

        Returns:
            CVAT mask format (flattened mask + bbox coordinates)
        """
        x1, y1, x2, y2 = map(int, box)

        # Extract mask region within bounding box
        mask_region = mask[y1:y2+1, x1:x2+1]

        # Flatten the mask and convert to list
        flattened_mask = mask_region.flatten().tolist()

        # Append bounding box coordinates
        flattened_mask.extend([x1, y1, x2, y2])

        return flattened_mask

    def post_process_outputs(self, outputs, original_shape):
        """
        Post-process ONNX model outputs into structured results.

        Args:
            outputs: Raw ONNX model outputs [detection_output, proto_masks]
            original_shape: Original image shape (height, width)

        Returns:
            Dictionary with processed detection and mask results
        """
        if not outputs or len(outputs) < 2:
            return {'boxes': [], 'scores': [], 'classes': [], 'masks': [], 'num_detections': 0}

        detection_output = outputs[0]  # [1, 61, 1344]
        proto_masks = outputs[1]       # [1, 32, 64, 64]

        # YOLOv8 ONNX output format: [batch, channels, num_detections]
        # Our model: [batch, 61, 1344] where 61 = 4 + 32 + 25 (boxes + mask_coeffs + classes)

        if len(detection_output.shape) != 3:
            raise ValueError(f"Expected 3D detection output, got {len(detection_output.shape)}D")

        batch_size, channels, num_detections = detection_output.shape

        if batch_size != 1:
            raise ValueError(f"Expected batch size 1, got {batch_size}")

        # Extract data for single batch
        predictions = detection_output[0]  # Shape: [channels, num_detections]
        predictions = predictions.T  # Transpose to [num_detections, channels]

        # Extract components based on channel structure
        if channels == 61:  # Segmentation model
            boxes = predictions[:, :4]  # x_center, y_center, width, height
            class_scores = predictions[:, 4:29]  # 25 class scores
            mask_coeffs = predictions[:, 29:]  # 32 mask coefficients
        else:
            # Fallback for standard detection format
            boxes = predictions[:, :4]
            class_scores = predictions[:, 4:]
            mask_coeffs = None

        # Convert from center format to corner format
        x_center, y_center, width, height = boxes[:, 0], boxes[:, 1], boxes[:, 2], boxes[:, 3]
        x1 = x_center - width / 2
        y1 = y_center - height / 2
        x2 = x_center + width / 2
        y2 = y_center + height / 2

        boxes_corner = np.column_stack([x1, y1, x2, y2])

        # Get best class and confidence for each detection
        max_scores = np.max(class_scores, axis=1)
        max_classes = np.argmax(class_scores, axis=1)

        # Filter by confidence threshold
        conf_mask = max_scores >= self.conf_threshold

        if not np.any(conf_mask):
            return {'boxes': [], 'scores': [], 'classes': [], 'masks': [], 'num_detections': 0}

        filtered_boxes = boxes_corner[conf_mask]
        filtered_scores = max_scores[conf_mask]
        filtered_classes = max_classes[conf_mask]
        filtered_mask_coeffs = mask_coeffs[conf_mask] if mask_coeffs is not None else None

        # Apply NMS
        nms_indices = self.apply_nms(filtered_boxes, filtered_scores, self.iou_threshold)

        if not nms_indices:
            return {'boxes': [], 'scores': [], 'classes': [], 'masks': [], 'num_detections': 0}

        # Select final detections
        final_boxes = filtered_boxes[nms_indices]
        final_scores = filtered_scores[nms_indices]
        final_classes = filtered_classes[nms_indices]
        final_mask_coeffs = filtered_mask_coeffs[nms_indices] if filtered_mask_coeffs is not None else None

        # Scale boxes to original image size
        scaled_boxes = self.scale_boxes(final_boxes, original_shape)

        # Generate masks if we have mask coefficients
        masks = []
        if final_mask_coeffs is not None and len(proto_masks.shape) == 4:
            masks = self.generate_masks(
                final_mask_coeffs,
                proto_masks[0],  # Remove batch dimension [32, 64, 64]
                scaled_boxes,
                original_shape
            )

        return {
            'boxes': scaled_boxes,
            'scores': final_scores,
            'classes': final_classes,
            'masks': masks,
            'num_detections': len(final_boxes)
        }

    def infer(self, image, threshold):
        """
        Run inference on an image.

        Args:
            image: PIL Image
            threshold: Confidence threshold for filtering detections

        Returns:
            List of detection results in CVAT format with masks
        """
        self.conf_threshold = threshold

        # Preprocess image
        input_tensor, original_shape = self.preprocess_image(image)

        # Run ONNX inference
        input_name = self.input_details[0]
        outputs = self.model.run(self.output_details, {input_name: input_tensor})

        # Post-process results
        results_dict = self.post_process_outputs(outputs, original_shape)

        # Convert to CVAT format
        results = []
        if results_dict['num_detections'] > 0:
            boxes = results_dict['boxes']
            scores = results_dict['scores']
            classes = results_dict['classes']
            masks = results_dict['masks']

            orig_h, orig_w = original_shape

            for i, (box, score, cls) in enumerate(zip(boxes, scores, classes)):
                x1, y1, x2, y2 = box

                # Ensure coordinates are within image bounds
                xtl = max(int(x1), 0)
                ytl = max(int(y1), 0)
                xbr = min(int(x2), orig_w)
                ybr = min(int(y2), orig_h)

                # Skip invalid boxes
                if xtl >= xbr or ytl >= ybr:
                    continue

                # Create mask annotation if masks are available
                if i < len(masks):
                    mask = masks[i]
                    cvat_mask = self.mask_to_cvat_format(mask, [xtl, ytl, xbr, ybr])

                    results.append({
                        "confidence": float(score),
                        "label": self.labels.get(int(cls), f"class_{int(cls)}"),
                        "points": [],  # Empty for mask type
                        "mask": cvat_mask,
                        "type": "mask",
                    })
                else:
                    # Fallback to rectangle if no mask available
                    results.append({
                        "confidence": float(score),
                        "label": self.labels.get(int(cls), f"class_{int(cls)}"),
                        "points": [xtl, ytl, xbr, ybr],
                        "type": "rectangle",
                    })

        return results
