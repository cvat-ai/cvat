...
class ModelHandler:
    def __init__(self):
        ...
        base_dir = os.path.abspath(os.environ.get("MODEL_PATH", "/opt/nuclio/ritm"))
        model_path = os.path.join(base_dir, "coco_lvis_h32_itermask.pth")
        ...
        for k in state_dict.keys():
            if 'feature_extractor.stage2.0.branches' in k:
                self.net = utils.load_hrnet_is_model(state_dict, self.device, backbone)
                break

        if self.net is None:
            self.net = utils.load_deeplab_is_model(state_dict, self.device, backbone)
