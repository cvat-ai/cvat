...
class ModelHandler:
    def __init__(self):
        ...
        base_dir = os.path.abspath(os.environ.get("MODEL_PATH", "/opt/nuclio/clickseg"))
        model_path = os.path.join(base_dir)

        self.net = None
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'

        checkpoint_path = utils.find_checkpoint(model_path, "seg_formerB3-S2.pth")
        self.net = utils.load_single_is_model(checkpoint_path, self.device)
    ...
    def handle(self, image, pos_points, neg_points, threshold):
        ...
        predictor = get_predictor(self.net, 'FocalClick', device=self.device)
