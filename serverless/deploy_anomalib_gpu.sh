#!/bin/bash
# Deploy all GPU-enabled Anomalib-based Nuclio functions in a folder
...
# Find all function-gpu_*.yaml files in the given directory
find "$DEPLOY_DIR" -maxdepth 1 -type f -name "function-gpu_*.yaml" | while read -r yaml_file; do
