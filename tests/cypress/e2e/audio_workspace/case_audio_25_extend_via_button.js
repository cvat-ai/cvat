    ...
    describe(`Testing case "${caseId}"`, () => {
        it('Extend button creates a new region from the end of the last region to the playhead', () => {
            cy.get('body').then(($body) => {
                ...
                // Advance the playhead past the just-created region's end. The
                // audio-player nav hotkeys (Space/Right/Alt+Right) refused to
                // fire under headed Cypress through cy.type, realPress and even
                // direct KeyboardEvent dispatch — so we leave CREATE mode
                // (Esc -> cursor) and click on the waveform's far-right area;
                // wavesurfer's default behavior is to seek the playhead to the
                // click position.
