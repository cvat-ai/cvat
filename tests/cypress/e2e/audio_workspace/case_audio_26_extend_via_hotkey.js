    ...
    describe(`Testing case "${caseId}"`, () => {
        it('Shift+E creates an extend region using the cached/active label without opening the popover', () => {
            cy.get('body').then(($body) => {
                ...
                // Use Create-via-button to establish an active label so the
                // Extend hotkey path takes the no-popover branch.
                cy.audioCreateRegionViaButton(firstLabelName, 100, 250);
                cy.get('.cvat-audio-region-item').should('have.length', initial + 1);
                // See case_audio_25 — advance the playhead by clicking the
                // waveform in cursor mode (audio nav hotkeys aren't reachable
                // via synthetic events in headed Cypress).
