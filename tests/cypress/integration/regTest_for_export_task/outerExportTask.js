...
Cypress.Commands.add('exportOuterTask', ({
    taskName,
    archiveCustomName,
    order,
    imageClick = false,
    ...
    const archFileNameFin = `${archiveCustomName}_${taskName}_${orderId}`;
    ...
    cy.get('.cvat-modal-export-option-item')
        .eq(orderId)
        .click();
    cy.get('.cvat-modal-export-task').find('.cvat-modal-export-filename-input').type(archFileNameFin);
...
context('Export task outer.', { browser: '!firefox' }, () => {
    const caseId = '999';
    const labelName = 'block';
    const taskName = `Case test outer ${caseId}`;
        ...
        exportOrders.forEach(
            (ord) => {
                ...
                it(`Export a task without image, number ${ord}`, () => {
                    cy.exportOuterTask(parms); // without image
                });

                it(`Export a task with image, number ${ord}`, () => {
                    cy.exportOuterTask({ ...parms, imageClick: true }); // with image
                });
            },
        );
    });
