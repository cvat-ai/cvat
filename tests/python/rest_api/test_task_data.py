...
@pytest.mark.with_external_services
@pytest.mark.timeout(900)  # 15 minutes - allow for slow cloud storage downloads
@pytest.mark.parametrize("cloud_storage_id", [2])  # Use private bucket to simulate slow downloads
@pytest.mark.parametrize("use_cache", [False])
def test_task_creation_with_slow_cloud_storage_download_does_not_timeout(
    self,
    cloud_storage_id: int,
    use_cache: bool,
    cloud_storages,
    ...
    """
    Regression test for PR #10904.

    This test ensures that task creation from cloud storage with use_cache=False
    does not timeout when:
    1. Registry cleanup is running from non-main threads (fixed by PR #10904)
    2. Cloud storage downloads take a long time (requires adequate RQ timeout)

    Previously, the combination of:
    - PR #10904 enabling proper registry cleanup
    - Default 4-hour IMPORT_DATA timeout
    - Slow cloud storage downloads without cache

    Would cause legitimate task creation jobs to be marked as "abandoned" and cleaned up.

    This test simulates a scenario that takes a considerable time to download,
    ensuring the job stays alive and completes successfully.
    """
    cloud_storage = cloud_storages[cloud_storage_id]

    # Generate a larger dataset to ensure the download takes time
    # Use at least 10-15 images to simulate a realistic scenario
    s3_client = s3.make_client(bucket=cloud_storage["resource"])

    # Create test images in cloud storage
    images = generate_image_files(
        15,  # Enough files to take some time
        sizes=[(800, 600)] * 15,  # Reasonably sized images
    )

    test_prefix = "test_slow_download/"
    server_files = []
    ...
    for image in images:
        ...
        s3_client.create_file(
            data=image,
            filename=f"{test_prefix}{image.name}",
        )
        server_files.append(f"{test_prefix}{image.name}")
        request.addfinalizer(
            partial(
                s3_client.remove_file,
                filename=f"{test_prefix}{image.name}",
    ...
    task_spec = {
        "name": f"Task from cloud storage {cloud_storage_id} without cache (slow download test)",
        "labels": [{"name": "test"}],
    }

    data_spec = {
        "image_quality": 75,
        "use_cache": use_cache,  # False - forces full file download
        "cloud_storage_id": cloud_storage_id,
        "server_files": server_files,
    }

    # This should NOT timeout or be marked as abandoned
    task_id, _ = create_task(self._USERNAME, task_spec, data_spec)
    ...
    with make_api_client(self._USERNAME) as api_client:
        task, response = api_client.tasks_api.retrieve(task_id)
        assert response.status == HTTPStatus.OK
        assert task.size == len(images), f"Expected {len(images)} frames, got {task.size}"

        # Verify all frames are accessible
        data_meta, _ = api_client.tasks_api.retrieve_data_meta(task_id)
        assert data_meta.size == len(images)

        # Spot check: retrieve first chunk to ensure data is actually present
...
@pytest.mark.parametrize("cloud_storage_id", [2])
def test_task_creation_heartbeat_prevents_abandoned_status(
    self,
    cloud_storage_id: int,
    cloud_storages,
    request,
    ...
    """
    Regression test for PR #10904 - Verify RQ job heartbeat mechanism.

    This test ensures that during long-running cloud storage downloads,
    the RQ job sends periodic heartbeats to prevent being marked as abandoned.

    The test uses monkeypatching to track heartbeat calls and verify they occur
    ...
    heartbeat_calls = []
    original_heartbeat = rq.job.Job.heartbeat

    def tracked_heartbeat(self, *args, **kwargs):
        heartbeat_calls.append((args, kwargs))
        return original_heartbeat(self, *args, **kwargs)

    # Create test data
    s3_client = s3.make_client(bucket=cloud_storage["resource"])
    images = generate_image_files(10, sizes=[(800, 600)] * 10)
    test_prefix = "test_heartbeat/"
    server_files = []

    for image in images:
        image.seek(0)
        s3_client.create_file(data=image, filename=f"{test_prefix}{image.name}")
        server_files.append(f"{test_prefix}{image.name}")
        request.addfinalizer(
            partial(s3_client.remove_file, filename=f"{test_prefix}{image.name}")
        )

    task_spec = {
        "name": "Heartbeat verification test",
        "labels": [{"name": "test"}],
    }
    ...
    # Patch heartbeat to track calls
    with patch.object(rq.job.Job, 'heartbeat', tracked_heartbeat):
        task_id, _ = create_task(self._USERNAME, task_spec, data_spec)

    # Verify task completed
    with make_api_client(self._USERNAME) as api_client:
        task, _ = api_client.tasks_api.retrieve(task_id)
        assert task.size == len(images)

    # Verify heartbeats were sent during the operation
    # (The exact number depends on implementation, but there should be at least some)
    assert len(heartbeat_calls) > 0, (
        "Expected heartbeat calls during long-running task creation, "
        "but none were detected. This could lead to jobs being marked as abandoned."
    )