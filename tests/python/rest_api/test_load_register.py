...
def get_admin_user(admin_user: str):
    # getting the admin user from session fixture
    return admin_user
...
class PerformTask(locust.SequentialTaskSet):
    ...
    def perform_random_user_register_task(self):
        # Initializing Faker for name/email generation
        fake = Faker()
...
class LoadUser(locust.HttpUser):
    ...
    tasks = [PerformTask.perform_random_user_register_task]  # a list of tasks to perform
