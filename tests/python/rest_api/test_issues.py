...
class TestListIssues:
    def _test_can_see_issues(self, user, data, **kwargs):
        results = get_paginated_collection(user, "issues", **kwargs)

        assert DeepDiff(data, results) == {}

    def test_admin_can_see_all_issues(self, issues):
        self._test_can_see_issues("admin2", issues.raw)
    ...
    def test_can_filter_by_org_id(self, field_value, query_value, issues, jobs):
        ...
        self._test_can_see_issues("admin2", list(issues), org_id=query_value)
