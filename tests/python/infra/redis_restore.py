...
class RedisStateRestorer:
    ...
    Observed speedup in local benchmark runs on the PR4B branch:
    - `restore_redis_inmem_per_function`: ~53.1 ms -> ~0.34 ms (~156x faster)
    - `restore_redis_ondisk_per_function`: ~54.7 ms -> ~0.20 ms (~278x faster)
    """
