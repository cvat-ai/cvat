...
class CommonRequestHandlerClass(BaseHTTPRequestHandler, ABC):
    ...
    def check_query(self, query_params):
        ...
        if query_params.get('response_type', 'code') != supported_response_type:
            self.send_response(400)
            self.wfile.write('Only code response type is supported by dummy auth server'.encode("utf8"))
            return
    ...
    def do_GET(self):
        ...
        if u.path == self.AUTHORIZE_PATH:
            ...
            token = self.headers.get('Authorization') or dict(parse_qsl(u.query)).get('access_token')
...
class GithubRequestHandlerClass(CommonRequestHandlerClass):
    ...
    def authorize(self, query_params):
        ...
        # add query params
        new_query = f"?code={generated_code}&state={query_params['state']}&" \
    ...
    def generate_access_token(self):
        ...
        generated_token = ''.join(sample(string.ascii_letters + string.digits, GITHUB_AUTH_TOKEN_LENGTH))
